...
CALENDAR_URL = "https://norton.ticketapp.org/portal/product/17/calendar"
TIME_SLEEP = 15
...
def check_date_open(target_date: datetime) -> bool:
    """Check if a specific date is clickable (open) in the datepicker."""
    month_value = str(target_date.month - 1)  # jQuery UI: 0=Jan ... 11=Dec
    ...
    try:
        ...
        logger.info(f"Selected month {target_date.strftime('%B')}")

        wait.until(EC.invisibility_of_element_located(
            (By.CSS_SELECTOR, ".event_loader")))

        year_select = Select(driver.find_element(
            By.CSS_SELECTOR, "select.ui-datepicker-year"))
        year_select.select_by_value(year_value)
        logger.info(f"Selected year {year_value}")

        wait.until(EC.invisibility_of_element_located(
            (By.CSS_SELECTOR, ".event_loader")))
        time.sleep(1)  # small buffer, AJAX render can lag after spinner hides

        clickable_days = driver.find_elements(
            By.CSS_SELECTOR,
            "table.ui-datepicker-calendar td a.ui-state-default",
        )
        open_days = {a.text.strip() for a in clickable_days if a.text.strip()}

        logger.info(f"Open days this month: {sorted(open_days, key=int)}")

        is_open = day_text in open_days
...
def send_alert_email(target_date: datetime):
    ...
    subject = f"Norton Museum - {date_str} is open!"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Check if a date is open on Norton calendar")
    parser.add_argument(
        "--date",
        required=True,
